...
class Mage_Sales_Model_Order_Invoice extends Mage_Sales_Model_Abstract
{
    ...
    // WING2010
    function getDoormatAmountToCapture() {
      $base_price = $this->getBaseGrandTotal();
      ...
      foreach($all_items as $item) {
        ...
        if(!empty($product)) {
          ...
          if(intval($stock) <= 0) {
            $base_price = $base_price - $item->getBaseRowTotal() + 1.0;
          }
        }
      }

      $deferredShipping = Mage::getSingleton('core/session')->getDelayedShipping();
      if(!empty($deferredShipping)) {
        $base_price -= $deferredShipping;
      }
