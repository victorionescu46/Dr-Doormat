...
class Mage_Sales_Model_Quote_Address extends Mage_Customer_Model_Address_Abstract
{
    ...
    protected function _construct()
    {
        ...
        $this->doormat_items = null;
        $this->backorderShippingAmounts = array();
    }
    ...
    public function getAllItems() {
      $type = $this->doormat_items;
      if(empty($type)) {
        return $this->getAllItemsOriginal();
      } else if($type == 'normal') {
        return $this->getAllNormalItems();
      } else if($type == 'backorder') {
        return $this->getAllBackorderItems();
      }
    }

    public function getAllItemsOriginal()
    {
     ...
     * @return Mage_Sales_Model_Quote_Address
     */
    public function addShippingRate(Mage_Sales_Model_Quote_Address_Rate $rate, $addRates = false)
    {
        $rate->setAddress($this);
        ...
        if(!$addRates) {
          ...
          foreach($rates as $arate) {
            if($arate->getCode() == $rate->getCode() && !$arate->isDeleted()) {
              $arate->setPrice($arate->getPrice() + $rate->getPrice());
              $exists = true;
              break;
            }
          }
        ...
        if($this->doormat_items == 'backorder') {
          $code = $rate->getCode();
          $price = $rate->getPrice();
          $this->backorderShippingAmounts = array_merge($this->backorderShippingAmounts, array($code => $price));
        }
    ...
    public function getAllNormalItems() {
      $all = $this->getAllItemsOriginal();
      $new_all = array();
      ...
      foreach($all as $item) {
        ...
        if(!empty($product)) {
          if($product->getStockItem()->getStockQty() <= 0) {
            continue;
          }
        }
    ...
    public function getAllBackorderItems() {
      $all = $this->getAllItemsOriginal();
      $new_all = array();
    ...
    public function collectShippingRates()
    {
        ...
        Mage::getSingleton('core/session')->setDelayedShipping(0);

        $normal_items = $this->getAllNormalItems();
        $backorder_items = $this->getAllBackorderItems();

        $found = false;
        if(!empty($normal_items)) {
          $this->doormat_items = 'normal';
          $this->collectTotals();
          $found1 = $this->requestShippingRates(null, true);
        }
        if(!empty($backorder_items)) {
          $this->doormat_items = 'backorder';
          $this->collectTotals();
          $found2 = $this->requestShippingRates(null, true);
        }
     ...
     * @return bool
     */
    public function requestShippingRates(Mage_Sales_Model_Quote_Item_Abstract $item = null, $addRates = false)
    {
        ...
        if ($result) {
            ...
            foreach ($shippingRates as $shippingRate) {
                ...
                if (!$item) {
                    $this->addShippingRate($rate, $addRates);
                }
                ...
                if ($this->getShippingMethod() == $rate->getCode()) {
                    if ($item) {
                        ...
                        if(isset($this->backorderShippingAmounts[$rate->getCode()])) {
                          Mage::getSingleton('core/session')->setDelayedShipping($this->backorderShippingAmounts[$rate->getCode()]);
                        }
                    }
